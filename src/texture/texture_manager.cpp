...
TextureManager::TextureManager(const GLchar* vertex_shader, const GLchar* fragment_shader)
...
void TextureManager::load(const char* file_path, std::string name) {
    textures[name] = Texture2D();
    textures[name].generate(file_path);
};

Shader TextureManager::get_shader() {
    return this->shader;
}

Texture2D TextureManager::get_texture(std::string key) {
