...
public:
    ...
    template <typename T>
    void
    set(const std::string &name, T value) const;

private:
    // compile shaders
    bool
    compile_shader(const char * vsrouce, const char * fsource, unsigned int &id);

    // load shader programs from file
    bool
    load_shaders(const char * vfile, const char * ffile, unsigned int &id);
};
